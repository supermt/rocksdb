...
namespace rocksdb {
  ...
  Status Env::ValidateLinkPoints(DBOptions options) {

    int level_pointer = 3; // move from 3 to 0, for PM,
    // NVMeSSD, SATASSD, STATHDD
    int layouts = options.storage_layouts;

    for (int i = level_pointer; i > 0; i--) {
      if (layouts >> i & 1) {
        // create target point
        // validate the options, but not now
//        connect_points.put("current layouts' link point");
      }
    }
    // here are the temp code
    std::string pm_dir = "";
    std::string nvme_ssd_dir = "/home/supermt/rocksdb_nvme";
    std::string sata_ssd_dir = "";
    std::string sata_hdd_dir = "/media/supermt/hdd/dataset";
    connect_points["pm_dir"] = "";
    connect_points["nvme_ssd_dir"] = nvme_ssd_dir;
    connect_points["sata_ssd_dir"] = "";
    connect_points["sata_hdd_dir"] = sata_hdd_dir;

    return Status::OK();
  }

  void Env::SetStorageLayouts(int layouts) { Env::storage_layouts = layouts; }
  ...
  namespace {  // anonymous namespace
    ...
    void AssignEnvOptions(EnvOptions *env_options, const DBOptions &options) {
      ...
      // add by jinghuan
      env_options->storage_layouts = options.storage_layouts;
    }
  ...
  EnvOptions Env::OptimizeForHybridStorage(
      const rocksdb::EnvOptions &env_options,
      const rocksdb::ImmutableDBOptions &db_options) const {
    EnvOptions optimized_env_options(env_options);
    optimized_env_options.storage_layouts = db_options.storage_layouts;
    return optimized_env_options;
  }
