...
namespace rocksdb {
    ...
    const int kLeftShiftPosNum = 9; // number,3 bit path_id, 3 bit source, 3 bit target
    const int kLeftShiftPosPath = 6;
    const int kLeftShiftPosSource = 3;
    const uint64_t kFileNumberMask = 0xFFFFFFFFFFFFFFF >> kLeftShiftPosNum;//0x3FFFFFFFFFFFFFFF;


    extern uint64_t PackFileNumberAndPathId(uint64_t number, uint64_t path_id, int source, int target);
    ...
    struct FileDescriptor {
        ...
        FileDescriptor(uint64_t number, uint32_t path_id, uint64_t _file_size,
                ...
                : table_reader(nullptr),
                  packed_number_and_path_id(PackFileNumberAndPathId(number, path_id, 0, 0)),
                  file_size(_file_size),
                  smallest_seqno(_smallest_seqno),
                  largest_seqno(_largest_seqno) {}

        FileDescriptor(uint64_t number, uint32_t path_id, uint64_t _file_size, int output_level)
                : FileDescriptor(number, path_id, _file_size, kMaxSequenceNumber, 0) {
            this->source_level = output_level;
            packed_number_and_path_id = PackFileNumberAndPathId(number, path_id, output_level, 0);
        }
...
//                    >> (64 - kLeftShiftPosPath);
        ...
        int GetTarget() const {
//            return (packed_number_and_path_id << (64 - kLeftShiftPosSource))
