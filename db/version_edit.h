...
const int kLeftShiftPosNum =
    9;  // number,3 bit path_id, 3 bit source, 3 bit target
const int kLeftShiftPosPath = 6;
const int kLeftShiftPosSource = 3;
const uint64_t kFileNumberMask =
    0xFFFFFFFFFFFFFFF >> kLeftShiftPosNum;  // 0x3FFFFFFFFFFFFFFF;
...
struct FileDescriptor {
  ...
  FileDescriptor(uint64_t number, uint32_t path_id, uint64_t _file_size,
      ...
      : FileDescriptor(number, path_id, 0, 0, _file_size, _smallest_seqno,
                       _largest_seqno) {}

  FileDescriptor(uint64_t number, uint32_t path_id, uint64_t _file_size,
                 uint16_t output_level)  // this constructor is for compaction
                                         // creation only
      : FileDescriptor(number, path_id, 0, output_level, _file_size,
                       kMaxSequenceNumber, 0) {}

  FileDescriptor(uint64_t number, uint32_t path_id, uint16_t _source_level,
                 uint16_t _target_level, uint64_t _file_size,
                 SequenceNumber _smallest_seqno, SequenceNumber _largest_seqno)

      : table_reader(nullptr),
        packed_number_and_path_id(PackFileNumberAndPathId(
            number, path_id, _source_level, _target_level)),
  ...
  FileDescriptor &operator=(const FileDescriptor &fd) {
    ...
    source_level = fd.source_level;
    target_level = fd.target_level;
    packed_number_and_path_id = PackFileNumberAndPathId(
        fd.GetNumber(), fd.GetPathId(), source_level, fd.target_level);

    return *this;
  }

  uint64_t GetNumber() const {
    //            return packed_number_and_path_id >> kLeftShiftPosNum;
    return rocksdb::GetNumber(packed_number_and_path_id);
  }

  uint32_t GetPathId() const {
    return rocksdb::GetPathId(packed_number_and_path_id);
  }

  int GetSource() const {
    //            return (packed_number_and_path_id << (64 - kLeftShiftPosPath))
    //                    >> (64 - kLeftShiftPosPath);
    return rocksdb::GetSource(packed_number_and_path_id);
  }

  int GetTarget() const {
    //            return (packed_number_and_path_id << (64 -
    //            kLeftShiftPosSource))
    //                    >> (64 - kLeftShiftPosSource);
    return rocksdb::GetTarget(packed_number_and_path_id);
  }
...
class VersionEdit {
 public:
  ...
  void AddFile(int level, uint64_t file, uint32_t file_path_id,
               ...
               int _source_level = 0, int _target_level = 0) {
    assert(smallest_seqno <= largest_seqno);
    FileMetaData f;
    f.fd = FileDescriptor(file, file_path_id, _source_level, _target_level,
                          file_size, smallest_seqno, largest_seqno);
    f.smallest = smallest;
    f.largest = largest;
