...
// we don't know this field.
    ...
    // add by jinghuan
    // use the last three bits to store the level
    uint64_t PackFileNumberAndPathId(uint64_t number, uint64_t path_id, int source_level, int target_level) {
        assert(number <= kFileNumberMask);
        return (number << kLeftShiftPosNum) | (path_id << kLeftShiftPosPath) |
               (source_level << kLeftShiftPosSource) | target_level;
    }
