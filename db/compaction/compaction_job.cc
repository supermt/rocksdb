...
// Maintains state for the entire compaction
    ...
    Status CompactionJob::FinishCompactionOutputFile(
        ...
        if (meta != nullptr) {
            ...
            fname =
                    TableFileName(sub_compact->compaction->immutable_cf_options()->cf_paths,
                                  meta->fd.GetNumber(), meta->fd.GetPathId(),
                                  Env::GetLinkDirectory(output_fd.GetSource()));
            output_fd = meta->fd;
        } else {
            fname = "(nil)";
        }
    ...
    Status CompactionJob::OpenCompactionOutputFile( // here is where to open the file
            SubcompactionState *sub_compact) {
        assert(sub_compact != nullptr);
        assert(sub_compact->builder == nullptr);
        // no need to lock because VersionSet::next_file_number_ is atomic
        uint64_t file_number = versions_->NewFileNumber();
        int out_put_level = sub_compact->compaction->output_level();
        std::string fname =
                TableFileName(sub_compact->compaction->immutable_cf_options()->cf_paths,
                              file_number, sub_compact->compaction->output_path_id(),
                              Env::GetLinkDirectory(out_put_level));
        // Fire events.
        ColumnFamilyData *cfd = sub_compact->compaction->column_family_data();
#ifndef ROCKSDB_LITE
