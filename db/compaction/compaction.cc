...
//    std::string result = "";
        ...
        for (int l = 0; l < vstorage->num_levels(); l++) {
            ...
            for (const auto &file: files) {
                stringStream << file->fd.GetPathId() << file->fd.GetNumber() << "\t";
            }
            stringStream << "\n";
        }
        return stringStream.str();
...
//    std::cout << "compaction triggered with style: " << vstorage->GetCompactionStyle() << std::endl;
        std::cout << "compaction triggered to level: " << output_level() << std::endl;

//        std::cout << GetStorageHierarchyString(vstorage) << std::endl;
//    std::cout << GetStorageFileString(vstorage) << std::endl;
