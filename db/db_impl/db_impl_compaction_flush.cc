...
Status DBImpl::CompactFilesImpl(
  ...
  //    std::cout << "Compaction Finished, Inputs: ";
  //    for (auto infile : input_files) {
  //      std::cout << "[Level: " << infile.level << ",";
  //      for (auto file : infile.files) {
  //        std::cout << file->fd.GetPathId() << file->fd.GetNumber() << " ";
  //      }
  //      std::cout << "]";
  //    }
  //    std::cout << ", Outputs in level: " << output_level;
  //
  //    for (std::string filename : *output_file_names) {
...
Status DBImpl::ReFitLevel(ColumnFamilyData *cfd, int level, int target_level) {
  ...
  //   std::cout << "Refitting the levels, from level: " << target_level
  //             << " to level: " << to_level << std::endl;
  return status;
}
    ...
    *made_progress = true;
    ...
    for (unsigned int l = 0; l < c->num_input_levels(); l++) {
      ...
      for (size_t i = 0; i < c->num_input_files(l); i++) {
        ...
        uint64_t fd_number = f->fd.GetNumber();
        uint32_t fd_pathid = f->fd.GetPathId();

        c->edit()->DeleteFile(c->level(l), fd_number);
        // modified by jinghuan
        // the file has been deleted by level l, record it's previous level for
        // chasing the old file

        f->fd.source_level = l;

        int out_level = c->output_level();

        c->edit()->AddFile(out_level, f->fd.GetNumber(), f->fd.GetPathId(),
                           f->fd.GetFileSize(), f->smallest, f->largest,
                           f->fd.smallest_seqno, f->fd.largest_seqno,
                           f->marked_for_compaction,
                           // add by jinghuan
                           f->fd.GetSource(), f->fd.GetTarget());
        // add by jinghuan
        f->fd.packed_number_and_path_id =
            PackFileNumberAndPathId(fd_number, fd_pathid, l, out_level);
        f->fd.target_level = out_level;
        ROCKS_LOG_BUFFER(
            log_buffer,
