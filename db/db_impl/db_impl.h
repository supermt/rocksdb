...
// Class to maintain directories for all database paths other than main one.
  ...
  private:
    ...
    int storage_layouts = 1;
    std::vector<std::unique_ptr<Directory>> storage_hierarchies_dirs;
...
// divided in several db_impl_*.cc files, besides db_impl.cc.
  ...
  protected:
    ...
    enum AssignOrder : bool {
      kDontAssignOrder, kDoAssignOrder
    };
    // Whether it requires publishing last sequence or not
    enum PublishLastSeq : bool {
      kDontPublishLastSeq, kDoPublishLastSeq
    };
