...
namespace rocksdb {
  ...
  Status Directories::SetDirectories(Env *env, const std::string &dbname,
    ...
    // the initialization of the storage directories.
...
// REQUIRES: log_numbers are sorted in ascending order
    ...
    for (auto log_number : log_numbers) {
      if (log_number < min_log_number) {
        ROCKS_LOG_INFO(immutable_db_options_.info_log,
                       "Skipping log #%"
                           PRIu64
                           " since it is older than min log to keep #%"
                           PRIu64,
      ...
      if (!status.ok()) {
        ...
        if (immutable_db_options_.wal_recovery_mode ==
          ...
          ROCKS_LOG_INFO(immutable_db_options_.info_log,
                         "Point in time recovered to log #%"
                             PRIu64
                             " seq #%"
                             PRIu64,
  ...
  Status DBImpl::Open(const DBOptions &db_options, const std::string &dbname,
    ...
    *dbptr = nullptr;
    ...
    // Modifications made for detecting hybrid architecture
