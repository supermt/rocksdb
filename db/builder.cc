...
namespace rocksdb {
  ...
  Status BuildTable(// this function can be called only by flushing the Level0 Tables
    ...
    // add the level into the last character
    // which means the level should no longer than 1 character
    // but actually this won't influence that much
    // reason is simple, when it reaches 10, you will need a PB to store one sstable
    // Serious?
    std::string link = "";
    if (env->storage_layouts != 1) {
      // there is link points
      // and here is always the 0 level, so link equals to the highestlevel
      link = env->GetLinkDirectory(level);
    }

    std::string fname = TableFileName(ioptions.cf_paths, meta->fd.GetNumber(),
                                      meta->fd.GetPathId(), link);
    // add by jinghuan
    // no need to condiser the files here, they are all level 0
//    std::cout << "create the file :" << fname << std::endl;
    // but since the files are all in Level0, that means it should be stored
...
//    std::cout << "Creating Writeable File in name : " << fname << std::endl;
// Creating Writeable File in name : /tmp/rocksdbtest-1000/dbbench/000050.sst
// it get the result here, which means the file name has already been changed here.
