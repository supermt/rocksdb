...
namespace rocksdb {
    ...
    const static int kPosNum = 9; // number,3 bit path_id, 3 bit source, 3 bit target
    const static int kPosPath = 6;
    const static int kPosSource = 3;

    inline uint64_t GetNumber(uint64_t packed_number_and_path_id) {
        return packed_number_and_path_id >> kPosNum;
    }
...
// this option configs the second storage are hybrid architecture or not.
// if this option is set to true, be sure the next option need be more
// than one.
//  bool is_hierarchy; // modified by Jinghuan

// this option configs how many directories we need to create for the system
// the way we adapt to hybrid system is very simple, we create enough directories
// in the db directory, and mount or link the target storage material to
// target directory.
        int storage_layouts = 0; // modified by Jinghuan
...
//        static uint64_t GetNumber(uint64_t packed_number_and_path_id);
//
//        static uint32_t GetPathId(uint64_t packed_number_and_path_id);
//
//        static int GetSource(uint64_t packed_number_and_path_id);
//
//        static int GetTarget(uint64_t packed_number_and_path_id);


        static int GetStorageLayouts();

        static Status ValidateLinkPoints(DBOptions options);

        static void SetStorageLayouts(int layouts);
        ...
        virtual EnvOptions OptimizeForHybridStorage(
                const EnvOptions &env_options,
                const ImmutableDBOptions &db_options) const;
